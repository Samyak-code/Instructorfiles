...
def update():
    ...
    if len(queue) > 0:
        ...
        if abs(angle) == 1 and speed == 1:
            ...
            current_linear_velocity = abs(ang_vel[1])*WHEEL_DISTANCE/math.sin(math.pi/4)
        # If we are driving straight, update distance left to travel
        if abs(angle) == 0 and speed == 1:
            current_linear_velocity = old_linear_velocity + lin_acc[2]*rc.get_delta_time()
            dist_left = queue[0][0] - (current_linear_velocity*rc.get_delta_time())
